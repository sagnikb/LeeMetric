...
%%
\begin{abstract}
  We give a generalization of the Fourier analytic proof of the binary Linear Programming bound due to Navon and Samorodnitsky to more general channels - the q-ary Hamming metric and the Lee metric, for codes that are closed under inverses. We also derive a numerical bound on the size of the Lee ball of a given radius. 
\end{abstract}
...
Much of information theory involves studying the limits of reliable communication over a channel, using well-designed codes. We can use various metrics defined on the codeword space that are matched to the channel under consideration, where we say that a metric is matched to a channel if nearest neighbour decoding in the codeword space implies ML decoding on the channel. The most well studied metric is the Hamming metric, and we know a lot about this metric. In the random noise model (or asymptotic reliability setting), we have a precise characterisation of the capacity of the channel, with any rate below capacity being achievable and the probability of error going to zero asymptotically, and conversely, any rate above capacity implying that the probability of error goes asymptotically to one. We also know the rate at which the probability goes to one (error exponents), computationally efficient codes coming close to capctity for many channels etc. For adversarial noise (zero-error information theory) the capacity is not precisely known, as the best known upper bound (the second LP bound) and the best know lower bound (the GV bound) on the achievable rates do not match. 

There are other metrics of interest too, for example the Lee metric. Substantially less is known about the Lee metric than about the Hamming metric case, even though there has been a lot of work on this metric, for example...
